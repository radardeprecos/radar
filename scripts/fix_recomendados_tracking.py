...
old = 'onclick="Radar de PreçosAuth.trackProductClick(${JSON.stringify(\'\').replace(\'""\',\'snapshot(product)\')})" href="${safe(url(product))}">Ver oferta</a></article>`; }).join(\'\'); }\n    async function fav(id){'
new = 'onclick="trackRec(\'${product.id}\')" href="${safe(url(product))}">Ver oferta</a></article>`; }).join(\'\'); }\n    function trackRec(id){ const p=products.find(x=>String(x.id)===String(id)); if(p) Radar de PreçosAuth.trackProductClick(snapshot(p)); }\n    async function fav(id){'
if old not in html:

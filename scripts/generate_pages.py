...
def generate_product_page(product: Dict[str, Any], template_path: str, output_dir: str) -> None:
    logger.info(f"Gerando página para o produto: {product.get("name")}")
    ...
    product_price = f"R$ {product.get("price", 0):.2f}".replace(".", ",")
    product_original_price = f"R$ {product.get("originalPrice", 0):.2f}".replace(".", ",")
    ...
    # Conteúdo SEO e descrição (simulado por enquanto, será gerado por LLM no futuro)
...
def generate_all_product_pages(input_path: str, template_path: str, output_dir: str) -> None:
    ...
    logger.info(f"Total de {len(products)} páginas de produtos geradas.")

if __name__ == "__main__":
    # Exemplo de uso
    # Certifique-se de que data/new_offers.json e templates/product_template.html existam

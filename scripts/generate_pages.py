...
def generate_product_page(product: Dict[str, Any], all_products: List[Dict[str, Any]], template_path: str, output_dir: str) -> None:
    ...
    try:
        ...
        orig_val = float(product.get('originalPrice', 0) or product.get('original_price', 0))
    ...
    p_savings = f"R$ {savings_val:,.2f}".replace(',', 'X').replace('.', ',').replace('X', '.')
    ...
    # LÓGICA DE MONETIZAÇÃO FORÇADA
    p_url = product.get('permalink') or product.get('url') or ''
    if 'mercadolivre.com' in p_url or 'mlb.com' in p_url:
        if 'matt_tool=' not in p_url:
            separator = '&' if '?' in p_url else '?'
            p_url = f"{p_url}{separator}matt_tool={ML_ACCOUNT}"
        else:
            p_url = re.sub(r'matt_tool=[^&]+', f'matt_tool={ML_ACCOUNT}', p_url)
    elif 'amazon.com' in p_url:
        if 'tag=' not in p_url:
            separator = '&' if '?' in p_url else '?'
            p_url = f"{p_url}{separator}tag={AMAZON_TAG}"
        else:
            p_url = re.sub(r'tag=[^&]+', f'tag={AMAZON_TAG}', p_url)
    ...
    status_html = ""
    if p_status == 'expired':
        status_html = '<div style="background: #ffebee; color: #c62828; padding: 15px; border-radius: 8px; margin-bottom: 20px; font-weight: 600;">⚠️ Esta oferta encerrou, mas confira produtos similares abaixo!</div>'
    
    # Buscar similares da mesma categoria
    similars = [p for p in all_products if p.get('custom_category_slug') == p_cat_slug and str(p['id']) != p_id and p.get('status') == 'active']
    random.shuffle(similars)
    similars = similars[:4]
    
    similars_html = ""
    if similars:
        similars_html = '<div class="similars"><h2>Produtos Similares em Oferta</h2><div class="similars-grid">'
        for s in similars:
    ...
    content = content.replace('{{product.savingsAmount}}', p_savings)
...
def generate_all(input_p: str, temp_p: str, out_d: str) -> None:
    ...
    logger.info(f"Gerando {len(all_products)} páginas permanentes com novo design...")

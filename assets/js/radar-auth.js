...
(function () {
  ...
  const LOCAL_USER_KEY = 'radar_local_user';
  const LOCAL_DATA_PREFIX = 'radar_user_data_';
  const VIEW_HISTORY_KEY = 'radar_view_history';
  const CLICK_HISTORY_KEY = 'radar_click_history';
  ...
  function isFirebaseConfigured() {
    const config = window.RADAR_FIREBASE_CONFIG || {};
    const options = window.RADAR_FIREBASE_OPTIONS || {};
  ...
  function normalizeUser(user) {
    ...
    return {
      uid: user.uid,
      displayName: user.displayName || 'Usuário Radar',
      email: user.email || '',
  ...
  function notifyAuthChange() {
    window.dispatchEvent(new CustomEvent('radar:auth-change', { detail: { user: state.user, firebaseReady: state.firebaseReady } }));
  ...
  async function ensureProfile(user) {
    ...
    const root = (window.RADAR_FIREBASE_OPTIONS && window.RADAR_FIREBASE_OPTIONS.collectionsRoot) || DEFAULT_COLLECTIONS_ROOT;
  ...
  function saveLocal(collection, value) {
    ...
    window.dispatchEvent(new CustomEvent('radar:data-change', { detail: { collection, value } }));
    return value;
  }
    ...
    state.initPromise = (async () => {
      if (isFirebaseConfigured()) {
        try {
          ...
          if (!firebase.apps.length) firebase.initializeApp(window.RADAR_FIREBASE_CONFIG);
      ...
      renderHeaderAuth();
      window.addEventListener('radar:auth-change', renderHeaderAuth);
      return state.user;
    })();
  ...
  async function setCollection(collection, values) {
    ...
    if (state.firebaseReady && state.user && state.user.provider !== 'local') {
      ...
      await batch.commit();
      window.dispatchEvent(new CustomEvent('radar:data-change', { detail: { collection, value: normalized } }));
      return normalized;
    }
  ...
  function renderHeaderAuth() {
    ...
    headers.forEach(header => {
      let slot = header.querySelector('.radar-auth-widget');
      if (!slot) {
        slot = document.createElement('div');
        slot.className = 'radar-auth-widget';
        header.appendChild(slot);
      }
      const user = state.user;
      const label = user && !user.isAnonymous && user.provider !== 'local' ? (user.displayName || 'Conta') : 'Entrar';
      const photo = user && user.photoURL ? `<img src="${escapeHtml(user.photoURL)}" alt="${escapeHtml(label)}">` : '<span class="radar-auth-avatar">👤</span>';
      slot.innerHTML = `
        <a class="radar-auth-link" href="${relativePath('minha-lista/')}">Minha lista</a>
        <button class="radar-auth-button" type="button" data-auth-action="${user && user.provider !== 'local' ? 'logout' : 'login'}">${photo}<span>${escapeHtml(label)}</span></button>
      `;
